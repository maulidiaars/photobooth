...
interface StepTrackerProps {
  current: 1 | 2 | 3;
  /** "light" (default) is the original cream-page styling used on the
   *  camera/result screens. "maroon" re-tones every state to read on
   *  the deep-maroon backdrop (frame page). */
  theme?: "light" | "maroon";
}
 ...
 */
export function StepTracker({ current, theme = "light" }: StepTrackerProps) {
            ...
            <div className="flex flex-col items-center gap-1.5">
              <div
                className={clsx(
                  ...
                  maroon
                    ? clsx(
                        active && "bg-clay-gradient text-garnet-dark shadow-clay-sm",
                        done && "bg-paper-light/25 text-paper-light",
                        !active && !done && "bg-paper-light/10 text-paper-light/50"
                      )
                    : clsx(
            ...
            {step !== 3 && (
              <div
                className={clsx(
                  ...
                  maroon
                    ? done
                      ? "border-paper-light/40"
                      : "border-paper-light/15"
                    : done
                      ? "border-garnet/50"
                      : "border-ink/15"
                )}
