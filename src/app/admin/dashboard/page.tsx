...
import { getDashboardStats, getPhotos, updatePhotoStatus, deletePhoto, markPhotoNotified } from "@/services/photoService";
...
export default function AdminDashboardPage() {
    ...
    Promise.all([getDashboardStats(), getPhotos()])
      .then(([s, p]) => {
        ...
        if (opts.silent && knownIds.current.size > 0) {
          ...
          const newOnes = sorted.filter((x) => !knownIds.current.has(x.id));
          if (newOnes.length > 0) {
            toast.push(
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        <div>
          ...
          <p className="font-serif text-sm text-[#4A1A1A]/60 mt-0.5">
            Ringkasan aktivitas photobooth — otomatis update setiap {POLL_MS/1000} detik
          </p>
        </div>
      ...
      {/* Stats Cards */}
      {loading && <p className="font-serif text-sm text-[#4A1A1A]/60">Memuat statistik...</p>}

      {stats && (
        <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
          <StatsCard label="Total Frame" value={stats.totalFrames} icon={Images} bg="cream" />
          <StatsCard label="Total Foto" value={stats.totalPhotos} icon={Camera} bg="gold" />
          <StatsCard label="Pending Print" value={stats.pendingPrint} icon={Clock3} bg="creamDark" />
          <StatsCard label="Sudah Dicetak" value={stats.printed} icon={CheckCircle2} bg="maroon" />
        </div>
      )}
          ...
          <div className="rounded-2xl border border-[#E8DDD0]/50 bg-[#FBF7F2] p-4 sm:p-5 shadow-md">
            ...
            {filteredPhotos.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <p className="font-serif text-lg text-[#4A1A1A]/40">Belum ada foto</p>
                <p className="font-serif text-sm text-[#4A1A1A]/30">Foto akan muncul di sini setelah sesi selesai</p>
              </div>
            ) : (
              <>
                {/* Photo Grid */}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="rounded-2xl border border-[#E8DDD0]/50 bg-[#F5EBE0] p-4 sm:p-5 shadow-md">
              <div className="mb-3 flex items-center gap-2">
                <BarChart3 size={18} className="text-[#6B2D2C]" strokeWidth={2.2} />
                <h2 className="font-serif text-base font-semibold text-[#4A1A1A]">Frame Terpopuler</h2>
              </div>
              {frameBreakdown.length === 0 ? (
                <p className="font-serif text-xs text-[#4A1A1A]/50">Belum ada data.</p>
              ) : (
                <div className="flex flex-col gap-3">
                  {frameBreakdown.map((f, idx) => (
                    <div key={f.name}>
                      <div className="mb-1 flex items-center justify-between font-serif text-xs text-[#4A1A1A]/80">
                        <span className="truncate">{f.name}</span>
                        <span className="shrink-0 font-semibold text-[#6B2D2C]">{f.count}</span>
                      </div>
            ...
            {/* Action Needed */}
            <div className="rounded-2xl border border-[#E8DDD0]/50 bg-[#FBF7F2] p-4 sm:p-5 shadow-md">
              <h2 className="mb-2 font-serif text-base font-semibold text-[#4A1A1A]">Perlu Tindakan</h2>
              {(stats?.pendingPrint ?? 0) === 0 ? (
                <div className="flex items-center gap-2">
                  <CheckCircle2 size={16} className="text-[#5B7F5C]" strokeWidth={2.2} />
                  <p className="font-serif text-sm text-[#4A1A1A]/70">Semua sudah beres!</p>
                </div>
              ) : (
                <ul className="flex flex-col gap-2 font-serif text-sm text-[#4A1A1A]/80">
                  <li className="flex items-center gap-2 rounded-lg bg-[#C9A87C]/20 px-3 py-2">
                    ...
                    <strong className="text-[#6B2D2C]">{stats?.pendingPrint}</strong> foto menunggu cetak
                  </li>
                </ul>
              )}
            </div>
